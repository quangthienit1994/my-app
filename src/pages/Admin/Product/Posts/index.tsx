                                ...
                                this.state.posts.map((post: any) => {
                                    return (
                                        <GridListTile key={post.uid}>
                                            ...
                                            <GridListTileBar
                                                title={
                                                    post.deleting ? <LinearProgress /> : <Button className="p-0" color="inherit" style={{ textTransform: 'none' }} onClick={this.props.onChange(post.uid)}>
                                                        {post.title}
                                                    </Button>
                                                }
                                                        ...
                                                        <TableRow key={post.uid}>
                                                            ...
                                                            <TableCell>
                                                                <div className="d-flex align-items-center">
                                                                    <Typography className="mr-2 w-100 small">{moment(post.date).format('LLL')}</Typography>
                                                                    <Tooltip title="Xóa bài viết">
                                                                        <IconButton color="secondary" onClick={this.handleDeletePost(post)}><Delete /></IconButton>
                                                                    </Tooltip>
                                                                </div>
                    ...
                    if (author && author.id) {
                        author.get().then((auth: any) => {
                            if (auth.exists) {
                                ...
                                if (index !== -1) {
                                    postss[index].author = { ...auth.data(), uid: auth.id };
                                    this.setState({ posts: postss });
                                }
                            }
                        })
